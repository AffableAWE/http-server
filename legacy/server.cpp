...
void loadConfig() {
  ...
  if (env_port) {
    ...
    if (*end == '\0' && parsed > 0 && parsed <= 65535) {
      ...
      cerr << "[WARNING] Invalid PORT '" << env_port << "'. Using 8080."
           << endl;
    }
  }
}
...
/*
 * Removes leading and trailing whitespace (spaces, tabs, newlines) from the
 * given string. Input:- input: The string to trim. Returns:- A new string
...
string constructResponse(const string &status, const string &contentType,
                         const string &body) {
...
/*
 * handleRequest():
 * Handles a client connection, processes the request, and serves a static file
 * or a 404 error. Input:- client_fd: The file descriptor for the connected
 * client socket.
 */
void handleRequest(int client_fd) {
  ...
  while (true) {
    ...
    if (headers_end_pos != string::npos) {
      ...
      if (method != "GET") {
        string response = constructResponse("405 Method Not Allowed",
...
void worker_thread() {
  while (true) {
...

    {
      ...
      if (client_queue.empty() || !server_running)
        break;
...
int main() {
  ...
  if (bind(sockd4, (struct sockaddr *)&server_addr4, sizeof(server_addr4)) <
      0) {
  ...
  if (bind(sockd6, (struct sockaddr *)&server_addr6, sizeof(server_addr6)) <
      0) {
